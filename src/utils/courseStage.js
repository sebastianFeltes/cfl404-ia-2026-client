...
export function getCourseStageFromDates({ startDate, start_date, endDate, end_time, isAnnual, is_annual } = {}) {
  if (isAnnual || is_annual) return COURSE_STAGES.anual

  const startMonth = monthOf(startDate || start_date)
  const endMonth = monthOf(endDate || end_time)
